...
# BLOCKER PARAMETERS TYPE 3
lenB = 2.68 # length
initialVolB = lenB # Cell Volume
targetVolB = 2*initialVolB # Volume that triggers cell division
growB = 1 # Growth rate

# BLOCKER TRANSCONJUGANT PARAMETERS TYPE 4
lenBT = 2.68 # length
initialVolBT = lenBT # Cell Volume
targetVolBT = 2*initialVolBT # Volume that triggers cell division
growBT = 1 # Growth rate
...
cell_colors = {0:[0.0, 0.0, 1.0], 1:[1.0, 1.0, 0.0],2:[0.0, 1.0, 0.0], 3:[1.0, 0.0, 0.0], 4:[1.0, 0.0, 1.0]} # Array for color of cells: https://www.glowscript.org/docs/VPythonDocs/color.html
cell_lengths = {0:lenD, 1:lenR, 2:lenT, 3:lenB, 4:lenBT} # Array for cell lengths
cell_tarVol = {0:targetVolD, 1:targetVolR, 2:targetVolT, 3:targetVolB, 4:targetVolBT} # Array for target volumes
cell_growthRate = {0:growD, 1:growR, 2:growT, 3:growB, 4:growBT} # Array for growth rates
...
def setup(sim,invar=None):
    ...
    # Set plasmid transfer counters to 0
    sim.countHGTRecipient = True
    sim.HGTeventsRecipient = 0
    sim.countHGTBlocker = True
    sim.HGTeventsBlocker = 0
    ...
    # Add conjugation event
        if cell.cellType == 0: #if a cell is a donor
            ...
            if surrounding_types:
                ...
                if random.random() < cell.effGrowth*con_prob and cell_chosen_to_conjugate.cellType==1:
                    cell_chosen_to_conjugate.cellType=2
                    sim.HGTeventsRecipient += 1         
                if random.random() < cell.effGrowth*con_prob and cell_chosen_to_conjugate.cellType==3:
                    cell_chosen_to_conjugate.cellType=4
                    sim.HGTeventsBlocker += 1  
...
def divide(parent, d1, d2):
    ...
    if parent.cellType == 3:
        ...
        d2.length = lenB
        d2.growthRate = growB
    if parent.cellType == 4:
        d1.targetVol = targetVolBT
        d1.length = lenBT
        d1.growthRate = growBT
        d2.targetVol = targetVolBT
        d2.length = lenBT
        d2.growthRate = growBT